...
const aiScene *Engine::ToolsPrivate::openAsset(Assimp::Importer &importer, const GLchar *asset)
{     
     const aiScene *pScene = importer.ReadFile(asset, aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_CalcTangentSpace | aiProcess_JoinIdenticalVertices | aiProcess_OptimizeMeshes);
     if (!pScene)
     {
	  ALOGE("Failed to load model File: %s", asset);
